...
defineTypeNameAndDebug(rhoLGepsilon, 0);
addToRunTimeSelectionTable(RASModel, rhoLGepsilon, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

rhoLGepsilon::rhoLGepsilon
(
...
tmp<volSymmTensorField> rhoLGepsilon::devReff() const
{
...
tmp<fvVectorMatrix> rhoLGepsilon::divDevReff(volVectorField& U) const
{
...
tmp<fvVectorMatrix> rhoLGepsilon::divDevRhoReff
(
...
bool rhoLGepsilon::read()
{
    if (RASModel::read())
    {
        ...
        if (couplingFactor_.value() < 0.0 || couplingFactor_.value() > 1.0)
        {
            FatalErrorIn("rhoLGepsilon::read()")
                << "couplingFactor = " << couplingFactor_
                << " is not in range 0 - 1"
                << exit(FatalError);
        }
...
void rhoLGepsilon::correct()
{
    ...
    //- fvm::laplacian(rho_*DREff(), R_)
      + fvm::Sp((Clg1_+1.0)*rho_*epsilon_/k_, R_)
      ==
        rho_*P
    //- (2.0/3.0*(1 - Clg1_)*I)*rho_*epsilon_
      + (2.0/3.0*Clg1_*I)*rho_*epsilon_
      - Clg2_*rho_*dev(P)
